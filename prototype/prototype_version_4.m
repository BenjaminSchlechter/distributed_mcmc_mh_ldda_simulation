...
base_filename = "prototype_mcmc_hist"
...
show_pdf = 0;
show_pdf_from_inverse_function_samples = 0;

show_inverse_function = 0;
show_inverse_function_from_samples = 0;
...
if (1 == show_acceptance)
	...
	legend("acceptance per step", sprintf("overall acceptance rate: %f", acceptance_rate));
	xlabel("Markov chain iteration")
	ylabel("share of accepted changes")
...
if (1 == show_dichtefunktion_from_reference_histogram)
	...
	legend_str(legend_index) = "Histogram uniform samples"; legend_index++;
...
if (1 == show_dichtefunktion_from_iterative_histogram)
	...
	legend_str(legend_index) = "Histogram MCMC samples"; legend_index++;
elseif (2 == show_dichtefunktion_from_reference_histogram)
...
if (1 == show_dichtefunktion_rebalanced)
	...
	legend_str(legend_index) = "with correction"; legend_index++;
elseif (2 == show_dichtefunktion_rebalanced)
	...
	% semilogy(xvalues, exp(xvalues/T), "color", [0.6, 0.0, 0.0]);
	% legend_str(legend_index) = "correction bias"; legend_index++;

	semilogy([0:num_classes-1]/(num_classes-1), rebalanced/nf, "color", [0.0, 0.8, 0.0]);
	legend_str(legend_index) = "corrected MCMC histogram"; legend_index++;
endif
...
legend(legend_str(1:legend_index-1), "location", "east");
